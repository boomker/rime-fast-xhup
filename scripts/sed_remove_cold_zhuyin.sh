...
gsed -i -r '/呢.*ni/{/呢[(喃|子|绒)]|[毛呢|你]/!s/ni/ne/g}' "$1" 2>/dev/null
gsed -i -r '/给.*ji/{/给[(予|与)]|[供|补]给/!s/ji/gw/g}' "$1" 2>/dev/null
gsed -i -r '/没.*mo/{/没[(落|莫|摸|入|收)]|[(辱|吞|湮|隐|淹|沉|埋|鬼|覆|出|浸)]没/!s/mo/mw/g}' "$1" 2>/dev/null
...
gsed -n -r '/还.*hr/{/还(阳|击|价|钱|款|书|童|债|手|回|珠|我|清|原|贷|朝|魂|俗|乡|给)|(返|交|归|偿|退)还/!p}' dyzhr
...
awk -F'\t' '{x=index($1, "没");split($2, a, " ");{if(a[x]=="mo")print $0}}' "$1" >dyzmo
awk -F'\t' '{x=index($1, "还");split($2, a, " ");{if(a[x]=="hr")print $0}}' "$1" >dyzhr
...
awk -F'\t' '{x=index($1, "着");split($2, a, " ");{if(a[x]!="vo")print $0}}' "$1" | rg '(着落|着色|着重|着实|着手|着装|着陆|穿着|沉着|执着|衣着|附着|胶着)' >dyzvo
