...
- 支持一键展开超级简拼, 输入3个及以上字母编码字符后, 按下 `Ctrl+0` 会展开成简拼, 让成语短语显示在列表里
...
- 支持词条置顶, 按下 `Ctrl+t` 触发一键置顶功能, 同一编码下, 先置顶的词条优先级最高, 后置顶的次之, 立即见效
...
- 支持分号 `;` 前缀快捷输出符号, 比如输入 `;d` 自动上屏 `、` , `;j`上屏 `#`
- 支持 Emoji 表情智能显示, 首页表情降频到第6位; 支持应用(仅微信)内表情输出, 非应用内表情自动隐藏
...
- `date`, `time`, `today`, `week`, `/wd`, /`wt`, `/wk`, `/nl`, `/uts` 可以输出当天的日期, 时间, 星期, 农历, 时间戳。
  `/wqt` `/wzt` `/wmt` `/wht` `/wuz` `/wxz` `/wuy` `/wxy` 等可输出最近几天几周几月(昨天, 上周, 上个月等)的日期时间。
...
5. - Q: 形码辅助引导符可以去掉吗? 想直接使用辅助码, 不用引导符
   - A: 可以的, 取消 [这行](https://github.com/boomker/rime-fast-xhup/blob/main/flypy_xhfast.custom.yaml#L118) 注释即可
