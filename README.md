...
* 自带双拼扩展词库，由脚本生成，可在双拼加辅助码的输入模式下使用，但是扩展词库不带辅助码（似乎也没必要性）
* 过滤掉生僻字，lua 脚本实现，其他实现方案貌似都有缺点
* 支持中英文混合输入，英文单词词库文件选择了四六级常见词汇
* 支持多种常见格式的英文单词输入，如：`Tool`， `TOOL`， `tool`　
* 支持简拼，自定义短语实现，初始词频低，加了长词优选功能，提升在前三候选中
* 加入了长词优选功能，常见英文单词和成语可以排在第一页
...
### 前缀模式
* `oh` 前缀：小鹤双拼键位查询，该功能为双拼初学者提供。例如：敲 `acian`，可看到韵母 `ian` 对应的按键是 `m`。
* `od` 前缀：大写模式，可以输入连续的几个大写字母，不需要大写锁定/`Shift` 键。例如：敲 `aulgpl` 空格，即可输入“LGPL”。
* `oz` 前缀：单词模式，不仅可以敲完整的单词，也允许“简写”，省略掉除了首字母以外的所有元音字母（`aeiou`）。例如：敲 `awelevation` 或者 `awelvtn` 再加空格，即可输入“elevation”。
...
* `oe` 前缀：emoji 模式，注意这里需要按照英文输入。例如：敲 `aelaugh` 空格，即可输入“😂”。
* `oq` 前缀：临时全拼模式，用于一些长的词组。例如：敲 `aptlbchqzh` 空格，即可输入“螳螂捕蝉黄雀在后”。
	* 可以设置“使用扩展词典”，例如加入一些常见古诗词，详见 `flypy_zrmfast.custom.yaml` 里的说明。
* `oa` 前缀：部件组字模式（类似搜狗拼音的 u 拆字模式），其中部件按照小鹤双拼输入。例如，要输入“犇”字（它可以拆为“牛牛牛”），敲 `abnqnqnq` 空格即可，并看到这个字的编码是 `bf[nn`。
...
* `tl` 前缀：简易 LaTeX 公式。例如：敲 `al<<f,ff>>ooc0` 空格，即可输入“`$\langle f,\phi\rangle\propto 0$`”。
