...
- 小鹤双拼音码组合优化，采用 不对应汉字读音的字母组合 替换一些手指*移动不和谐* 的字母组合键，实现左右手对侧击键
...
- 支持 Emoji 表情智能显示, 首页表情降频到第六候选, 支持应用(仅微信)内表情输出, 非应用内表情隐藏
...
- `~` 前缀：部件组字模式 (类似搜狗拼音的 u 拆字模式)，其中部件按照小鹤双拼输入。例如，要输入“犇”字 (它可以拆为“牛牛牛”)，敲 `~nqnqnq` 空格即可，并看到这个字的拼音是 `ben`。
...
- 敲 `date`,  `time`, `today`, `week`, `/wd`, /`wt`, `/wk`, `/cnl`, 可以输入当天的日期, 时间, 星期, 农历。
