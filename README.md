...
- 支持一键展开简拼(仅限一些成语俗语), 按下 `Ctrl+q` 会优先将长词放到候选菜单前排
...
- 支持英文单词「自造词」, 在单词末尾输入 `\` 符号, 下次输入编码(重新部署后)即可直接得到生词
  另有 `~` 前缀造词模式, 支持非典型英文单词(含数字、符号、空格), 空格由逗号转义而来
...
- 支持单字模式, 按下 `Ctrl+s`  即可切换到四码(音形码)匹配单字优先
...
- 支持斜杠 `/` 过滤二字词组(匹配两位形码), 比如输入 `uiui/oz` 自动上屏「适时」
...
- 对 Rime 默认 Emacs 键位的一些补充:
  ...
  - `Control+a` 移动光标到输入码最前面,  `Control+e `(或者 `End`) 把光标移动到最后
...
- Trime(同文)要在 [GitHub 页面](https://github.com/osfans/trime) 下载最新版, 自带适配方案的同文键盘皮肤
...
13. - Q: `rL` 反查出来在字没有注音, 部署日志中提示缺少  `kMandarin` 反查词典文件
