...
- 自创音码键位左右并击规则，替换一些手指 _移动不和谐_ (别手)的小鹤音码组合，实现左右手对侧击键, 更有节奏感
...
- 支持中英互译, 按下 `Ctrl+y` 可将高亮的候选字词一键发送到`Easydict`翻译
...
- 方案名称为“飞鹤快拼”，它提供了小鹤双拼+形码辅助选字的输入方案，默认 **开启** 间接辅码（有引导符）。
  如果希望使用 直接辅码(无引导符)，可以在 `flypy_xhfast.custom.yaml` 文件里取消注释 [对应的行](./flypy_xhfast.custom.yaml#L116)。
