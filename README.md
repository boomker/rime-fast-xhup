...
* 自带双拼二字词组词库，由脚本生成，可在双拼加辅助码的输入模式下使用，但是扩展大词库不带辅助码（似乎也没必要性）

* 过滤掉生僻字，lua 脚本实现，过滤掉一些不能正常显示的表情(但会误伤一些表情)，其他实现方案貌似都有缺点

* 支持中英文混合输入，英文单词词库文件选择了四六级常见词汇

* 支持多种常见格式的英文单词输入，如：`Tool`， `TOOL`， `tool`　

* 支持简拼，成语、俗语、古诗首字母放入自定义短语实现，初始词频低，加了长词优选功能，提升在前三候选
...
* `oz` 前缀：单词模式，不仅可以敲完整的单词，也允许“简写”，省略掉除了首字母以外的所有元音字母（`aeiou`）。例如：敲 `ozelevation` 或者 `ozelvtn` 再加空格，即可输入“elevation”。
...
* `tl` 前缀：简易 LaTeX 公式。例如：敲 `tl<<f,ff>>ooc0` 空格，即可输入“`$\langle f,\phi\rangle\propto 0$`”。
