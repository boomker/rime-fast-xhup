...
local conf = {
    number_en_st = {
        "0st", "1nd", "2rd", "3th", "4th", "5th", "6th", "7th", "8th", "9th"
    },
    week_cn = {
        "星期日", "星期一", "星期二", "星期三", "星期四",
        "星期五", "星期六"
    ...
    while true do
        ...
        if (pattern ~= nil) then
            ...
            if (replace_index ~= nil) then
                ...
                if (flag == false and string.find(pattern, ".number_cn}$") ~= nil) then
                    -- 值：转换为中文，e.g. 0=>〇，1=>一，...
                    if (string.find(pattern, ".arith.number_cn}$") ~= nil) then
                        ...
                        -- 21 => 二一
                        replace_value = ""
                        for i = 1, string.len(tostring(_ni)) do
                            local _ni_digit = tonumber(string.sub(tostring(_ni), i, i))
                            replace_value = replace_value ..
                                                tool.convert_arab_to_chinese(
                                                    _ni_digit)
                        end
                    end
...
function translator.func(input, seg, env)
    ...
    if (input == "week" or input == "oxq") then
    ...
    if (input == "time" or input == "wsj") then
