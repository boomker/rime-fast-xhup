...
function translator.func(input, seg, env)
    ...
    if (not favor_items) and input:match("^" .. favorCmdPrefix)
        and (favor_cmd_length_range:match(#input)) then
...
function filter.func(input, env)
    ...
    if command_cands then
        ...
        table.sort(sorted_keys, function(a, b) return tonumber(a) < tonumber(b) end)
