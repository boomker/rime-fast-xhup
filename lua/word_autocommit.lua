...
function F.func(input, env)
	...
	if (caret_pos >= 4) and (table.find({ 4, 5 }, #preedit_code)) and preedit_code:match("^%l+%[%l%l?$") then
		for _, cand in ipairs(single_char_cands) do
			...
			if spelling_hints > 0 then
				...
				comment = (string.len(remain_shape_code) > 0) and string.format("~%s", remain_shape_code) or "~"
			end
	...
	if (caret_pos >= 6) and (table.find({ 6, 7 }, #preedit_code)) and string.find(preedit_code, "^%l+%[%l+$") then
		for _, cand in ipairs(tchars_word_cands) do
			...
			if
				(#tchars_word_cands == 1)
				and tchars_word_cands[cand.text]
				and (tonumber(utf8.codepoint(cand.text, 1)) >= 19968)
			then
				local cand_txt = append_space_to_cand(env, cand.text)
				env.engine:commit_text(cand_txt)
				context:clear()
	...
	if (#preedit_code == 8) and preedit_code:match("^%l+") and env.autocommit_on then
		...
		for _, cand in pairs(fchars_word_cands) do
			...
			if
				...
				and (caret_pos >= 6)
				and (when_done == 1)
				and ((#preedit_code / 2 == utf8.len(commit_text)) or (#preedit_code / 3 == utf8.len(commit_text)))
			then
