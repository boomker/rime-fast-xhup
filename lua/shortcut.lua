...
function processor.func(key, env)
	...
	if key_value:match("^%a$") and (not launcher["main_menu_selected_text"]) then
	...
	if key_value:match("^%a$") and (not launcher["second_menu_selected_text"]) then
		local index = 0
		local match_count = 0
...
return {
	processor = { init = launcher.init, func = processor.func },
	translator = { init = launcher.init, func = translator.func },
}
